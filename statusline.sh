...
#          MS_LINE_ONLY=1 prints only the prayer line (compose with another statusline).
...
# ═══════════════════════════════════════════════════════════════════════════
# PART 1 — prayer times, hijri date, dhikr
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Compose the prayer line: prayer · hijri · dhikr ──
ms_line=""
[ -n "$prayer_line" ] && ms_line="$prayer_line"
[ -n "$hijri" ] && [ -z "$MS_NO_HIJRI" ] && { [ -n "$ms_line" ] && ms_line+="$sep"; ms_line+="${gold}☪️ ${hijri}${reset}"; }
[ -n "$ms_line" ] && ms_line+="$sep"; ms_line+="${teal}📿 ${dhikr}${reset}"

# Compose-with-another-statusline mode: just the prayer line, no usage block.
...
# ═══════════════════════════════════════════════════════════════════════════
# PART 2 — usage statusline (model · git · ctx · cost · limits)
# Adapted from claude-code-statusline by Aleksander Dytko (MIT).
# ═══════════════════════════════════════════════════════════════════════════
...
#   5h / 7d limits:  green < 70% | yellow 70–90% | red ≥ 90%
#   Extra usage:     green < 50% | yellow 50–80% | red ≥ 80%
#   ⚡ on 5h/7d/extra: utilization ≥ 100 (plan limit hit, routing to extra billing)
...
# ─── Build output ────────────────────────────────────────────────────────────
sep=" ${dim}|${reset} "
out1=""      # stdin-based: model, git            (always available, per-session)
out_cost=""  # stdin-based: ctx, cost            (always available, per-session)
out2=""      # API-based:   5h, 7d, extra         (cached, shared across tabs)

# Append to out2 with auto-separator
append2() { [ -n "$out2" ] && out2+="$sep"; out2+="$1"; }

# Append to out_cost with auto-separator
append_cost() { [ -n "$out_cost" ] && out_cost+="$sep"; out_cost+="$1"; }
...
# Context window (cost line)
if [ "${STATUSLINE_SHOW_CONTEXT}" = "true" ]; then
    ctx_color=$(context_color "$pct_used")
    append_cost "${white}ctx${reset} ${orange}${used_tokens}/${total_tokens}${reset} ${dim}(${reset}${ctx_color}${pct_used}%${reset}${dim})${reset}"
fi
...
# ─── Render ──────────────────────────────────────────────────────────────────
# Line 1: prayer · hijri · dhikr
# Line 2: model | git
# Line 3: ctx | cost | 5h | 7d | extra
usage_out="$out1"
line3="$out_cost"
[ -n "$out2" ] && { [ -n "$line3" ] && line3+="$sep"; line3+="$out2"; }
[ -n "$line3" ] && usage_out+="\n$line3"
